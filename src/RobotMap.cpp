...
std::shared_ptr<AnalogInput> RobotMap::shooterShooterEncoder;
std::shared_ptr<PIDController> RobotMap::shooterShooterControl;
...
std::shared_ptr<Relay> RobotMap::succSucc1;
std::shared_ptr<Relay> RobotMap::succSucc2;
...
void RobotMap::init() {
    ...
    shooterShooterMotor.reset(new VictorSP(12));
    lw->AddActuator("Shooter", "ShooterMotor", std::static_pointer_cast<VictorSP>(shooterShooterMotor));
    
    shooterShooterEncoder.reset(new AnalogInput(0));
    lw->AddSensor("Shooter", "ShooterEncoder", shooterShooterEncoder);
    
    shooterShooterControl.reset(new PIDController(1.0, 0.0, 0.0,/* F: 0.0, */ shooterShooterEncoder.get(), shooterShooterMotor.get(), 0.02));
    lw->AddActuator("Shooter", "ShooterControl", shooterShooterControl);
    shooterShooterControl->SetContinuous(false); shooterShooterControl->SetAbsoluteTolerance(0.2); 
        shooterShooterControl->SetOutputRange(-1.0, 1.0);
    ...
    succSucc1.reset(new Relay(0));
    lw->AddActuator("Succ", "Succ1", succSucc1);
    
    succSucc2.reset(new Relay(1));
    lw->AddActuator("Succ", "Succ2", succSucc2);
    
    climberClimberMotor.reset(new Spark(4));
